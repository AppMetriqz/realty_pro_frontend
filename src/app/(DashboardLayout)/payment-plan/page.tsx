    ...
    return (
        <PageContainer title="Planes de pagos" description="este es Planes de pagos">
            ...
            <Box sx={{ mx: '60px', mt: '40px' }}>
                <TableShared<Partial<GetPaymentPlanDto> & { id: number, phone_number_1:string, sale_type:string }>
                    headTitle="Pagos Atrasados"
                    headCells={HeadCellsPaymentPlan}
                    count={usePageProps.paymentPlanOverdue.isSuccess ? usePageProps.paymentPlanOverdue.data.count : 0}
                    rows={
                        usePageProps.paymentPlanOverdue.isSuccess
                            ? usePageProps.paymentPlanOverdue.data.rows.map((items) => ({
                                ...
                                sale_type: items.payment_plan.sale_type,
                            }))
                            : []
                    }
            ...
            <Box sx={{ mx: '60px', mt: '40px' }}>
                <TableShared<Partial<GetPaymentPlanDto> & { id: number, phone_number_1:string, sale_type:string }>
                    ...
                    headCells={HeadCellsPaymentPlan}
                    count={usePageProps.paymentPlanPending.isSuccess ? usePageProps.paymentPlanPending.data.count : 0}
                    rows={
            ...
            <Box sx={{ mx: '60px', my: '40px' }}>
                <TableShared<Partial<GetPaymentPlanDto> & { id: number, phone_number_1:string, sale_type:string }>
                    ...
                    headCells={HeadCellsPaymentPlan}
                    count={usePageProps.paymentPlanFinancing.isSuccess ? usePageProps.paymentPlanFinancing.data.count : 0}
                    rows={
