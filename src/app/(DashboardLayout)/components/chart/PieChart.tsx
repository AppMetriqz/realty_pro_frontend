...
export const PieChart = ({ usePageProps }: FiltersProps) => {
  ...
  const available_unit =
    usePageProps.finances.data.status.available_unit?.amount ?? 0;
  const sold_unit = usePageProps.finances.data.status.sold_unit?.amount ?? 0;
  const reserved_unit =
    usePageProps.finances.data.status.reserved_unit?.amount ?? 0;

  const options = {
    ...usePageProps.pieOptions,
    accessibility: {
    ...
    <>
      ...
      >
        ...
        >
          <TextAmount
            name={'Disponible'}
            amount={available_unit}
            currency={usePageProps.currency}
            color={'#E7E7E7'}
          />
          <TextAmount
            name={'Vendido'}
  ...
  return (
    <Box width={'100%'} display={'flex'} alignItems="center">
      ...
      >
        ...
        >
          {currency}
          {formatter.format(_.toNumber(amount))}
        </Typography>
      </Box>
    </Box>
  );
